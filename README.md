...
### 🎤 K-POP 선행지표 탭 (2026-08-29 신설)

`scripts/fetch_kpop.py` (cron `20 6 * * *`) → `data/db/kpop.json` · 프런트 `static/kpop.js` (탭 `p_kpop`).
엔터 실적은 **앨범이 팔린다 → 수출로 잡힌다 → 분기 실적에 찍힌다** 순서라, 앞 두 단계가 실적 발표를 선행한다.

| 층 | 소스 | 주기 | 비고 |
|---|---|---|---|
| 앨범 판매 | 써클차트 `circlechart.kr/data/api/chart/album` (무인증 POST) | 주간 | TOP100 → 아티스트→소속사 매핑 합산. 집계 전인 당주는 합계 0 으로 내려와 잘라낸다(급감 오독 방지) |
| 음반 수출 | 관세청 `tradedata.go.kr` **HS 8523491040** | 월간 | ⚠ 4자리 `8523` 은 SSD·반도체 미디어가 섞여 **연 250억\$** 규모(실측) — 쓰면 안 된다. 10자리(2025년 3.02억\$)가 통상 인용되는 음반 수출 통계와 일치 |
| 팬덤 화력 | YouTube Data API v3 `channels.list` (`keys/youtube.txt`) | 일간 | 16채널 · 쿼터 1/건. 증가분(Δ)은 `kpop_hist.json` 누적분으로만 산출하고 없으면 '누적 중'(추정 금지) |
| 연동 종목 | 야후 일봉 1년 | 일간 | 직결(엔터4사)·팬덤(디어유)·공연(LYV)·스트리밍(SPOT)·간접(CJ ENM) |

채널ID는 search API 로 1회 확인해 상수화했다(이후 쿼터 소모 없음). 아티스트→소속사는 레이블을 모회사로 귀속하므로
(빅히트·플레디스·어도어·빌리프랩 → 하이브 등) **실적 방향의 대리지표**로만 쓴다.

### 🛍️ K-소비재 선행지표 탭 (2026-08-29 신설)

`scripts/fetch_kcons.py` (cron `25 6 * * *`) → `data/db/kcons.json` · 프런트 `static/kcons.js` (탭 `p_kcons`).
이상적 선행 흐름(검색→콘텐츠→장바구니→주문→리뷰·재구매→수출)에서 검색·쇼피·SNS는 공개 API 부재 —
무토큰으로 잡히는 **품목군별 수출(관세청·월간 3년) × 연동 종목 주가** 축만 구현. 국내 관심도는 Trends 탭이 보조.

테마 구성: K뷰티(3304 화장품·3305 헤어·3303 향수·3307 방향제·8543 미용기기(광범위-노이즈 주의)) ·
K푸드(1902 라면·2103 소스·121221 김·1704 당류과자·2202 음료·2208 주류) ·
K패션(4202 가방·9004 선글라스·61+62 의류) — 14품목군 · 연동 19종목(브랜드=직결, ODM·유통=물량 연동,
현지생산 비중 큰 기업은 수출 통계와 괴리 정상). 12개월 누적선 = 계절성 제거 추세.
