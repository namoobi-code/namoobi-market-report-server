...
HZ = 30
# (2026-08-26) 시차 상한 36 → 48. h개월 예측에는 시차≥h 지표만 쓸 수 있어, 상한 36 에서는
#   30개월 예측의 시차 후보가 7칸뿐이라 지표들이 차선의 차선 시차로 밀리며 계수 부호가 흔들렸다
#   (실측: 서울 27M→28M 에서 KOSPI 기여 -0.355→-0.096, 예측선 한 달 새 17% 점프).
#   3안 백테스트(18~30M 구간): B(48) 가 3개 지역 전승 —
#     서울 2.70→2.34%·방향 92.5→95.0 · 경기 2.53→2.45%·95.0→96.7 · 전국 3.62→3.25%.
#   장지평 감쇠안(C)은 오차 악화(경기 3.30%·전국 4.08%)로 기각.
MAXLAG = 48
