...
HZ = 30
# (2026-08-26 최종) 시차 상한 30 · λ 고정 1.0 — **안정 우선**(사용자 확정).
#   경위: 상한을 48 로 넓히자 백테스트 MAPE 는 최고(서울 2.31%·방향 96.7%)였지만
#   실제 예측선이 폭주했다(서울 30M raw +0.704 = +102%, 최대 |raw| 1.062 = +189%).
#   진단 — 근본 원인은 시차가 아니라 **과적합**: 지평별 회귀 표본이 69~83개인데 지표가 36개
#   (p/n≈0.5)라 λ=0.3 벌점으로는 계수가 폭주한다(실측표 아래).
#     서울: 상한30/λ0.3 2.91%·최대raw 0.256 | 상한36/λ0.3 2.74%·0.404 | 상한48/λ0.3 2.31%·1.062
#           상한30/λ1.0 3.14%·0.217 ← 채택 (오차 0.4%p 양보하고 예측선을 현실 범위로)
#     경기: 상한30/λ1.0 2.53%·0.287  전국: 상한30/λ1.0 3.42%·0.652
#   정확도만 보면 λ 작을수록 좋지만, 그 성적은 '평균적으로 맞음'이고 현재 시점 예측이
#   비현실적으로 튀면 화면 산출물로서 쓸모가 없다 — 안정성을 평가 기준에 함께 넣어 결정했다.
MAXLAG = 30
LAM = 1.0                                # 지역별 λ(relead) 대신 고정 — 과적합 억제
...
def main():
    ...
    for reg in R.SIDO:
        ...
        lam = LAM                        # (2026-08-26) 고정 1.0 — 과적합 억제(위 실측표 참조)
