...
BATTLES = [
    ...
    ("aicode", "A", "기업 AI 코딩 점유 (클로드코드·OpenAI·Copilot·Cursor)", "%", "반기",
     "LLM 매출의 최대 침투 경로 — 클로드코드 42→54% 급확대가 앤트로픽 매출 성장의 엔진(기사: 매출 80% 기업). "
     "Copilot·Cursor 는 기업 점유 수치 미공표 — 개발자 선호도(JetBrains 2026)로 보면 클로드코드 46 · 커서 19 · 코파일럿 9%, "
     "클로드코드 직장 도입률 3%(25-04)→18%(26-01) 6배",
     [("클로드코드(앤트로픽)", "IPO 예정"), ("OpenAI", "MSFT(투자)"), ("GitHub Copilot", "MSFT"), ("Cursor", "비상장")],
     # (2026-09-02 감사 2차 · 사용자 지적 적중) 이전 판의 Copilot 70%(2023)→42%(2025)는 SEO 통계
     # 집적 사이트발로 **매출 실측과 모순** — 2026 ARR: 클로드코드 $2.5B(26-02·출시 9개월) ·
     # Cursor $2B(26-03·$1B에서 4개월 만에 2배) · Copilot ~$1B(유료 4.7M석) · 시장 $12.8B(E).
     # 매출 기준 Copilot 은 ~8% 수준이지 42%가 아니다 → 도구 축을 ARR 기준 점유(E)로 교체.
     [("2025-07", {"클로드코드(앤트로픽)": 42, "OpenAI": 21}, "Menlo 모델 축 실측 — 클로드 42% 첫 보도(TechCrunch)"),
      ("2025-11", {"Cursor": 14, "GitHub Copilot": 12}, "도구 축 ARR 추정(E) — Cursor $1B·Copilot ~$0.8B / 시장 ~$7B"),
      ("2026-03", {"클로드코드(앤트로픽)": 20, "Cursor": 16, "GitHub Copilot": 8},
       "도구 축 ARR 추정(E) — 클로드코드 $2.5B·Cursor $2B·Copilot ~$1B / 시장 $12.8B"),
      ("2026-06", {"클로드코드(앤트로픽)": 54, "OpenAI": 21}, "Menlo 모델 축 — 클로드코드 과반(42→54)")],
     None, "모델 축=Menlo 기업 코딩 워크로드(반기 실측) · 도구 축=공표 ARR/시장규모 역산(E) — 분모 다름(추세 비교용). JetBrains 선호도 46/19/9 보조"),
