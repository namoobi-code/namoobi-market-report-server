...
BATTLES = [
    ...
    ("ess", "A", "ESS 배터리 셀 출하 점유", "%", "월간",
     "LFP 전환의 승자 판별 — CATL 고착 vs 한국 2사 반격 여부가 LG엔솔·삼성SDI 밸류 결정",
     [("CATL", "300750.SZ"), ("BYD", "002594.SZ"), ("테슬라", "TSLA"), ("LG엔솔", "373220.KS")],
     [("2024-12", {"CATL": 37}, "(E)"),
      ("2025-12", {"CATL": 39}, "5년 연속 1위 실측 — BYD 신규설치 테슬라 추월")],
     None, "SNE리서치·BNEF 월간 보도"),
    ("cn_ev", "A", "중국 NEV 소매 점유", "%", "월간",
     "월간 인도량이 가장 고빈도 점유 데이터 — BYD 지배 지속 vs 테슬라 중국 침식이 두 주가의 핵심 변수",
     [("BYD", "002594.SZ"), ("테슬라", "TSLA")],
     [("2024-12", {"BYD": 32, "테슬라": 6}, "(E)"),
      ("2025-12", {"BYD": 29, "테슬라": 5}, "(E)")],
     None, "중국승용차협회(CPCA) 월간 공시"),
    ...
    ("ai_chip", "B", "AI 가속기 비중 (GPU vs 커스텀 ASIC)", "%", "분기",
     "하이퍼스케일러가 ASIC 비중을 늘릴수록 엔비디아 멀티플 압박·브로드컴 수혜 — 유니버스 두 카드의 제로섬",
     [("GPU(엔비디아)", "NVDA"), ("커스텀 ASIC", "AVGO")],
     [("2024-12", {"GPU(엔비디아)": 90, "커스텀 ASIC": 10}, "(E)"),
      ("2025-12", {"GPU(엔비디아)": 85, "커스텀 ASIC": 15}, "(E)")],
     None, "모건스탠리·JPM 추정 — 전량 기관 추정치(E)"),
    ("foundry", "B", "파운드리 매출 점유", "%", "분기",
     "선단공정 수주 배분 — TSMC 고착이 유지되는 한 TSM 멀티플의 바닥 논리",
     [("TSMC", "TSM"), ("삼성 파운드리", "005930.KS")],
     [("2024-12", {"TSMC": 67.1, "삼성 파운드리": 8.1}, "트렌드포스 실측"),
      ("2025-03", {"TSMC": 67.6, "삼성 파운드리": 7.7}, "트렌드포스 실측")],
     None, "트렌드포스 분기 보도"),
    ("tester", "B", "반도체 테스터 점유", "%", "반기",
     "HBM4 테스터 승부 — 어드밴 우위 유지 여부가 오늘 ⚪⚠ 판정의 검증 변수",
     [("어드밴테스트", "6857.T"), ("테라다인", "TER")],
     [("2025-12", {"어드밴테스트": 58, "테라다인": 35}, "(E)")],
     None, "양사 실적·업계 리서치"),
    ("cloud", "B", "클라우드 인프라 점유", "%", "분기",
     "3강 성장률 격차가 리레이팅 결정 — 알파벳 카드(GCP)의 핵심 변수",
     [("AWS", "AMZN"), ("Azure", "MSFT"), ("GCP", "GOOGL")],
     [("2024-12", {"AWS": 30, "Azure": 21, "GCP": 12}, "시너지리서치 실측 근사"),
      ("2025-12", {"AWS": 29, "Azure": 22, "GCP": 12}, "(E)")],
     None, "시너지리서치 분기 발표"),
    ("mlcc", "B", "MLCC 점유 (무라타·삼성전기)", "%", "반기",
     "AI 서버용 공급자 우위 시장의 양강 — 유통가 30% 인상 국면에서 점유 이동 여부",
     [("무라타", "6981.T"), ("삼성전기", "009150.KS")],
     [("2025-12", {"무라타": 40, "삼성전기": 23}, "(E)")],
     None, "업계 통설·양사 실적"),
