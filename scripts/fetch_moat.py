...
UNIV = [
    ...
    # (2026-09-20 편입 · 한경BUSINESS 2026.09.16-22 하나증권 '증설의 시대, 후공정의 수혜도 함께 온다')
    #   편입 근거는 점유율이 아니라 **마진의 지속성**이다 — 포고핀 글로벌 70% 에 영업이익률이
    #   2022 42.4% → 2023 44.8% → 2024 44.7% → 2025 상반기 46.3% 로 4년 내내 40%대다.
    #   결정적으로 2023 1Q 실적쇼크(매출 −22%·영익 −30%) 때도 OPM 38% 를 지켰다 —
    #   사이클 수혜주라면 불가능한 패턴이라 가격결정력이 공급자 쪽에 있다는 뜻.
    #   선행지표는 MU 가 아니라 ^SOX: 고객이 퀄컴(22%)·엔비디아·브로드컴 등 비메모리 중심이고
    #   940여개사로 분산돼 있어 메모리 업황 단일 대리로는 대표성이 떨어진다.
    #   ⚠ 같은 기사가 꼽은 나머지 후공정주는 전수 실측 후 탈락시켰다(코드 하단 RISKS·주석 참조).
    ("058470.KQ", "리노공업",       "AI반도체", "포고핀·테스트 소켓 글로벌 70% — 칩마다 맞춤 제작, 다운사이클에도 OPM 40%대", "^SOX", "필라델피아 반도체지수(SOX)"),
    ...
    # (2026-09-04 갱신) 노바티스 계약(최대 32.2억$)으로 올해 4건·플랫폼 누적 10건 —
    #   해자의 실체가 '머크 한 건'이 아니라 '빅파마가 반복해서 사 가는 플랫폼'으로 바뀌었다.
    #   선행지표도 MRK(단일 파트너) → HALO(동종 SC 제형 변환 플랫폼 업황)로 교체:
    #   고객이 4곳으로 늘어 단일 파트너 주가의 대표성이 떨어졌고, 이 시장 자체의 확장 여부가 더 중요해졌다.
    ("196170.KQ", "알테오젠",       "첨단바이오", "SC 제형 변환 플랫폼(하이브로자임/ALT-B4) — 빅파마 반복 채택, 2026년만 4건·누적 10건(AZ·GSK·노바티스 등)", "HALO", "할로자임(HALO — 동종 SC 제형 변환 플랫폼 업황·경쟁사)"),
...
SEL_B2 = {"SNPS","CDNS","6857.T","6981.T","6268.T","207940.KS","GEV","4063.T","267260.KS","298040.KS","LLY","AVGO",
          "058470.KQ"}  # (2026-09-20) 리노공업 — 출하 병목(테스트 소켓)형. 어드밴테스트·무라타와 같은 계열
...
MOAT_TYPES = {
    ...
    # 리노공업: 테스트 소켓은 칩마다 맞춤 설계·검증을 거치므로 양산 중 교체가 사실상 불가(switching),
    #   다품종 소량 정밀가공·도금 노하우가 규모의 경제로 복제되지 않는다(intangible)
    "058470.KQ": ["switching", "intangible"],
    ...
    "196170.KQ": ["intangible", "switching"], "VRTX": ["intangible"], "ILMN": ["switching", "intangible"],
    "CRSP": ["intangible"],
...
SHARES = {
    # (2026-09-05 기사 반영) 엔비디아 해자의 정량 증거 — 카운터포인트: 미·중 제외 55개국
    # 170개+ AI 모델 조사에서 '소버린 AI'의 92.4%가 엔비디아 가속기에 종속.
    "NVDA":      "소버린 AI 92.4%가 엔비디아 가속기 종속(카운터포인트 · 55개국 170개+ 모델) + AI 서버 매출 기준 ~86%(2025) — 각국이 데이터 주권을 원해도 하드웨어는 엔비디아라는 역설",
    ...
    "058470.KQ": "포고핀(프로브핀) 글로벌 ~70% — 2위권 격차는 미공표. 점유보다 마진 지속성이 근거: "
                 "OPM 2022 42.4%→2023 44.8%→2024 44.7%→2025 상반기 46.3%, 2023 1Q 실적쇼크에도 38% 유지",
...
RISKS = {
    # (2026-09-05 기사 반영) 엔비디아 — 해자 균열(탈엔비디아) 신호 추적: 韓 AI 스타트업의 AMD
    # 전환(모레 '라마-3-모티프-102B' AMD 기반 개발 · 업스테이지 MI355 GPU 도입)과 추론용 NPU
    # 대안(리벨리온 — 아람코 CVC 와에드벤처스 투자 유치). 단 소버린 AI 92.4% 종속(SHARES 참조)
    # 이라 현재는 균열보다 해자가 압도 — 이 목록이 길어지는 속도가 관전점.
    "NVDA": "탈엔비디아 시도 개시 — 韓 모레(AMD 기반 102B 모델)·업스테이지(AMD MI355 도입)·리벨리온 추론 NPU(아람코 CVC 투자). 아직 92.4% 종속이라 미미하나, 추론 시장에선 CUDA 락인이 학습보다 약해 균열 1순위 경로",
    "196170.KQ": "할로자임(ENHANZE)과 정면 경쟁 — 할로자임도 2026년 7월까지 신규 5건(GSK ADC·버텍스·인사이트 등)으로 목표(3건) 초과. 시장이 커지는 국면이라 동반 성장 중이나, 빅파마가 어느 플랫폼을 표준으로 굳히느냐가 장기 승부",
    ...
    "058470.KQ": "테스트 소켓 매출은 '신규 칩 설계 건수'에 연동 — 신제품 출시가 줄면 반복매출도 함께 준다(2023 1Q 매출 −22%). 다만 그 분기에도 OPM 38%를 지켜 마진 훼손형은 아니었다",
}

# ══════════════════════════════════════════════════════════════════════════════
# (2026-09-20) 국내 후공정 소부장 전수 심사 — 편입 1 / 탈락 4. 다시 검토하지 않도록 사유를 남긴다.
#   계기: 한경BUSINESS 2026.09.16-22 하나증권 '증설의 시대, 후공정의 수혜도 함께 온다'
#   (전환형 투자 → 증설형 투자로 사이클 이동, 후공정이 독립 투자축으로 부상).
#   ✅ 리노공업 — 위 UNIV 편입(선별B2). 근거는 점유율이 아니라 마진 지속성.
#   ❌ 고영(098460) — SPI 세계 1위 51%인데 OPM 16.1%→9.0%→1.6% 붕괴.
#        **'점유율 = 해자'가 아님을 보여주는 대표 반례**. 높은 점유에도 가격결정력이 없으면 해자가 아니다.
#   ❌ 넥스틴(348210) — "KLA 대항마" 서사와 달리 실제 DF 검사 점유 5~6%(KLA 90%+).
#        중국 매출 75~85%·고객집중 72%, OPM 41%→15%, 3개 분기 연속 적자. 진짜 독점자는 이미 있는 KLA.
#   ❌ 이오테크닉스(039030) — 레이저마킹 글로벌 60%는 성숙사업, 성장동력 어닐링은 특허 만료로 중국 진입 중.
#        OPM 10~22% 변동 = 설비투자 사이클 연동 장비주.
#   ⏳ ISC(095340) — 실리콘러버 소켓 ~90%·SLT ~50%·OPM 27~30%로 유망하나 SLT 복점이 '형성 중'이고
#        고객집중도·다운사이클 이력 미확인. **재심사 조건 2개: ① SLT 복점 고착 확인 ② 상위고객 비중 공시 확인.**
#   (미검증) 펨트론·인텍플러스·디아이·유니테스트·네오셈·와이씨·티에프이·덕산하이메탈·에이팩트·SFA반도체·에스티아이
#        — 조사 예산 내 미검증. 위 5종보다 해자가 강하다고 볼 근거는 찾지 못했다.
