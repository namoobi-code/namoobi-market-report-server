...
META = {
    ...
    "jr":       ("전세가율", "%", "가격", "M", "한국부동산원",
                 "매매가격 대비 전세가격 비율 · 높을수록 갭이 작아 매수 전환 유인이 커진다 · 2018.01~ · 시도별"),
    ...
    # (2026-08-22) 매경이코노미 커버스토리 — 보유세 인상분이 월세로 전가되는 경로 모니터링.
...
def main():
    ...
    # (2026-08-22) 전세가율 백필 — 구 표(DT_08002_07, 2018.01~2020.06)와 현행 표(_007A,
    #   2020.07~)는 같은 통계의 표 분할이라 정확히 이어진다(실측: 경계 202006/202007).
    #   ※ 월세지수는 2021.6 기준 개편으로 구 표와 기준이 달라 잇지 않는다 — 2021.06이 최대다.
    jr_old = kosis_monthly(408, "DT_08002_07", "T001", {"objL1": "ALL", "objL2": "0001"}, y0="201801")
    jr_new = kosis_monthly(408, "DT_08002_07_007A", "T001", {"objL1": "ALL", "objL2": "0001"})
    D["jr"] = {}
    for src in (jr_old, jr_new):                      # 현행 표가 나중에 덮는다
        for reg, mp in src.items():
            D["jr"].setdefault(reg, {}).update(mp)
    ...
    D["wolse"] = kosis_monthly(101, "DT_1YL20152E", "rent", {"objL1": "ALL", "objL2": "00"},
                               y0="202101")
