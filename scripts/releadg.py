...
def main():
    ...
    for i, code in enumerate(codes):
        ...
        _dump(T, names, out_pred, out_price, out_lead, out_d, partial=(i + 1 < len(codes)))

    _dump(T, names, out_pred, out_price, out_lead, out_d, partial=False)
    print(f"  → {OUT} ({OUT.stat().st_size // 1024}KB) · 지역 {len(out_pred)}")
    return 0


def _dump(T, names, out_pred, out_price, out_lead, out_d, partial):
    out = {
    ...
    tmp = OUT.with_suffix(f".json.tmp.{R.NOW.microsecond}")
