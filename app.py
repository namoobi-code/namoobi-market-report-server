...
def _us_rev_daily():
    ...
    try:
        t0 = _dt.date.today()
        t14 = (t0 + _dt.timedelta(days=7)).isoformat()   # (2026-09-02) 1주(사용자)
        byd = {}
        for r in pool.get("us") or []:
            ed = r.get("ed")
...
def kr_rev_daily(mkt: str = "kr"):
    ...
    try:
        ...
        try:
            ...
            pass
        t0 = _dt.date.today()
        t7 = (t0 + _dt.timedelta(days=7)).isoformat()   # (2026-09-02) 2주→1주 재조정(사용자)
...
# (2026-08-21) 광주·전남 통합으로 12xxx 신설 — 29(광주)·46(전남)은 과거 데이터가 남아 있어 함께 둔다
