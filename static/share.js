...
const CO_COLOR={
  ...
  // (2026-09-07 사용자 요청) AI 3카드 회사 색 통일 — 앤트로픽 계열(Anthropic·앤트로픽·클로드·클로드코드)=빨강,
  //   OpenAI=청록, 구글=남색, 딥시크=보라, 메타=하늘, xAI=회남, Cursor=금색, Copilot(MS)=초록, 기타(잔여)=회색
  'Anthropic':'#dc2626','앤트로픽':'#dc2626','클로드':'#dc2626',
  'OpenAI':'#0f766e','Google':'#4f46e5','구글':'#4f46e5','DeepSeek':'#7c3aed','딥시크':'#7c3aed',
...
function render(){
  ...
  $(P+'_grid').innerHTML=bs.map(b=>{
    ...
    return `<div style="flex:1 1 ${BIG?'100%':'480px'};max-width:${BIG?'none':'640px'};border:1px solid #e2e8f0;border-top:3px solid ${g[0]};border-radius:10px;background:#fff;padding:12px 14px">
      ...
      <div style="height:${BIG?'clamp(720px, 42vw, 1400px)':'210px'}"><canvas id="${P}_cv_${b.id}"></canvas></div>
      ${tbl}
