      ...
      show.map(i=>{
        ...
        const det=`<tr><td colspan="15" style="background:#f8fafc;padding:8px 14px">
          ...
          (i.ty||[]).map(t=>{
            ...
            <td class="num">${t.est!=null?F(t.est)+(t.act?' <b style="color:#0f766e" title="추정이 아니라 이 단지 자체의 입주 후 실거래(최근 12개월) ㎡당가 기반 실측 시세 — 단지명·준공년 매칭">실측</b>':'')+(t.nb?` <span class="note" title="같은 시군구 준공 10년 이내 신축 ${t.nb}개 단지의 실거래만으로 계산(구축 평균 왜곡 제거)">신축${t.nb}</span>`:'')+(t.estb?'<span title="신축 표본 부족 — 전 연식 실거래 ㎡당가 사용(보수적 추정)">†</span>':''):'—'}</td>
