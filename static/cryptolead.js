...
function fmtV(k,e){
  ...
  if(k==='funding'||k==='alt_funding') return (v>0?'+':'')+v.toFixed(4)+'%';
  if(k==='eth_btc') return v.toFixed(5);
...
const HELP={
 ...
 stable_ratio:{what:'스테이블코인 총공급 ÷ (BTC+ETH 시총). "대기 중인 달러가 시장 대비 얼마나 큰가".',read:'30일 변화율. −10% 이하 = 대기자금이 코인으로 들어감(위험선호) · +10% 이상 = 코인에서 스테이블로 대피(위험회피). 가격이 오르면 분모가 커져 같이 내려가므로 ±10% 넘는 큰 움직임만 본다.',low:'돈이 코인으로 투입되는 중 → 알트까지 온기가 퍼지는 국면.',high:'대피 중 → 알트가 가장 먼저 맞는다.'},
