...
(function(){
  ...
  function render(){
    ...
    $('re_ind').innerHTML=`<div style="text-align:right;margin:1px 0"><button id="re_rst" style="${bsty}" title="가중치 배수·시나리오 전부 초기화(전 지역 반영)">⟲ 조절 전체 초기화</button></div>
      ...
      HZS.map(h=>`<th style="text-align:right">${h}M</th><th style="text-align:right" title="${h}개월 예측에 쓰인 시차(≥${h}M)에서의 가격 전년비 상관 — 엔진이 실제로 고른 시차·r 그대로. 예전엔 h개월 누적변화율로 쟀는데 월별 누적은 창이 겹쳐 상관이 부풀려졌다(난수도 30M에서 0.396)">${h}M r</th>
      ...
      데이터 클러스터 9.51% — 전 지역·전 지평 압승으로 채택. 표의 <b>hM·hM r 은 엔진이 실제로 고른 시차와 그때의 상관</b>
      (가격 전년비 기준 — 2026-08-26 통일. 이전의 h개월 누적변화율 기준은 창이 겹쳐 상관이 부풀려졌다: 난수도 30M에서 0.396),
      각 지평의 <b>'가' 열이 그 지평 릿지 |β| 발언권</b>(지평 합계 100% · 조절 배수 반영)이다.
