...
(function(){
  ...
  function render(){
    ...
    $('re_ind').innerHTML=`<div style="text-align:right;margin:1px 0"><button id="re_rst" style="${bsty}" title="가중치 배수·시나리오 전부 초기화(전 지역 반영)">⟲ 조절 전체 초기화</button></div>
      ...
      "r × 현재 표준화값" 합성 원신호(+3점 평활·역사범위 가드). 가중치는 <b>그룹예산제</b>: 먼저 그룹별 총 발언권을
      그 그룹 합성계열의 |r_h|에 비례해 배정하고, 그룹 안에서 |r_h| 비례로 나눈다 — 같은 얘기를 하는 지표가 많아도
      그 주제의 총 발언권은 늘지 않는다(머릿수 과대반영 방지 · 지평별로 따로 계산). 보정계수는 곱하지 않는다 —
      선택/평가 분리 실측에서 보정이 오히려 성적을 해쳤다(서울 6.87%→9.00%). <b>통설 vs 실측 시차</b>가 크게 다르면
      통설이 이 지역 데이터에선 안 맞았다는 뜻. 기사 프레임: 전세→매매(1~2M) · 인허가→입주(6~18M) · 낙찰가율 80%↑ 안정/70%↓ 침체.</div>`;
