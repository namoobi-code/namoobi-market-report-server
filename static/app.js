...
(function(){
  ...
  document.querySelectorAll('.tab').forEach(b=>b.addEventListener('click',()=>{
    ...
    if(b.dataset.pane==='p_resh'&&window.renderResh) window.renderResh();   // (2026-09-20) 부동산시황 상승률 순위
    ...
    if(b.dataset.pane==='p_share2'&&window.renderShareWork) window.renderShareWork();  // (2026-09-07) 점유율 추이(업무)
    ...
    if(b.dataset.pane==='p_exp'&&window.renderExports) window.renderExports(); // (2026-09-05) K-수출
    if(b.dataset.pane==='p_cl'&&window.renderCryptolead) window.renderCryptolead(); // (2026-09-05) 코인 선행지표
    if(b.dataset.pane==='p_llm'&&window.renderLlm) window.renderLlm(); // (2026-09-10) AI 챗봇
...
(function(){
  ...
  const DEF={
    kr:{
      ...
      /* ③ 영업익 추정 리비전 — 컨센서스 스냅샷 차분. 30일=부분 창(쌓인 만큼) 즉시 · 90일=2026-11-07~ */
      ...
      /* (2026-09-02 사용자 요청) 전망 전구간 증가 — WISEreport 가 주는 모든 미래(E) 구간의
         영업익 증가율이 양수인가. 분기(E)=전년동기 대비 · 연간(E)=전년 대비. 1=전부+, 0=하나라도 ≤0 */
      qup:{label:'전망분기 전구간↑',fmt:v=>v?'전부 +':'미충족',reqData:1,
        presets:[['전체',null,null],['모든 분기(E) 증가',1,null]],def:[null,null]},
      yup:{label:'전망연간 전구간↑',fmt:v=>v?'전부 +':'미충족',reqData:1,
        presets:[['전체',null,null],['모든 연간(E) 증가',1,null]],def:[null,null]},
    ...
    us:{
      ...
      cr90:{label:'EPS 컨센 리비전 변화율 (90일)',fmt:v=>(v>0?'+':'')+v.toFixed(1)+'%',reqData:1,
        presets:[['전체',null,null],['상향 +5%↑',5,null],['상향 +10%↑',10,null],['하향 −5%↓',null,-5]],def:[null,null]},
      qup:{label:'전망분기 전구간↑',fixed:'— (KR 전용 · WISEreport 컨센 분기표 기반)'},
      yup:{label:'전망연간 전구간↑',fixed:'— (KR 전용 · WISEreport 컨센 연간표 기반)'},
      ...
      /* (2026-08-09) US 목표주가 리비전 — 미국 목표가는 '현재 유효한 목표가 평균'(기간 개념 없음 ·
         ...
         30일 = 부분 창(쌓인 만큼·최소 7일)으로 즉시 제공 · 90일 = 2026-11-07~ (부분 창 없음). */
      tprv:{label:'목표주가 리비전 30일',fmt:v=>(v>0?'+':'')+v.toFixed(1)+'%',reqData:1,
        presets:[['전체',null,null],['상향 +3%↑',3,null],['상향 +10%↑',10,null],['하향 −3%↓',null,-3],['하향 −10%↓',null,-10]],def:[null,null]},
      tprv90:{label:'목표주가 리비전 90일',fmt:v=>(v>0?'+':'')+v.toFixed(1)+'%',reqData:1,
        presets:[['전체',null,null],['상향 +5%↑',5,null],['상향 +10%↑',10,null],['하향 −5%↓',null,-5]],def:[null,null]},
  ...
  const KEYS=['mk','krx','wics2','wics','usind','sector','px','chg','cap','tv','de','cr','opLoss','age',
              ...
              'edld','spr','sspr','sprb','cr7','cr30','cr90','qup','yup','pr7','pr30','pr90','tprv','tprv90','gap','gapP','gapE','gapEP','r1','r20',
  ...
  const CDEF={
    ...
    cr90:{l:'추정90일',n:1,m:'both'}, qup:{l:'전망분기↑',n:1,m:'kr'}, yup:{l:'전망연간↑',n:1,m:'kr'},
  ...
  function colVal(r,k){
    switch(k){
      ...
      case 'qup': case 'yup': return r[k]!=null?r[k]:null;            // 전망 전구간 증가 플래그(1/0)
  ...
  function cell(r,key){
    ...
    switch(key){
      ...
      /* (2026-09-02) 전망 전구간 증가 — 모든 미래(E) 분기 YoY(qup) / 연간 전년比(yup) 양수 */
      case 'qup': return `<span class="${v?'up':'dn'}" title="WISEreport 분기표의 모든 미래(E) 분기 영업익이 전년동기 대비 증가하는가">${v?'✓ 전부+':'✗'}</span>`;
      case 'yup': return `<span class="${v?'up':'dn'}" title="WISEreport 연간표의 모든 미래(E) 연도 영업익이 전년 대비 증가하는가">${v?'✓ 전부+':'✗'}</span>`;
  ...
  function renderLegend(){
    ...
    {const rn=$('scr_revnote'); if(rn){
      const g = mkt==='us' ? [
        ...
        ['컨센 7·30·90일 리비전','진행분기·다음분기 EPS 컨센서스 추정치의 그 시점 대비 변화율(평균). <b>가이던스가 시장에 어떻게 소화됐나</b>의 결과다. +5%↑=애널리스트가 전망 상향. 미국은 즉시 산출, <b>한국(컨센30일=영업이익 리비전)은 가장 가까운 미래 추정 분기의 영업익 컨센 기준(같은 분기끼리), 스냅샷이 30일 미만이면 쌓인 만큼 부분 창으로 제공(적재 2026-08-09~)</b>. <b>주의</b>: 기저(그 시점 EPS 추정치)가 0 근처인 종목은 %가 크게 부풀 수 있어(예: 0.09→1.38 = +1452%, 실제 영향은 작음) 그런 경우 화면 값이 자동으로 <b>주가 대비 %p</b>로 바뀐다(아래 항목 참고) — 값은 정확하고, 표시 방식만 바뀌는 것'],
        ...
        ['영업익추정 리비전 30일·90일','영업이익 컨센서스 추정치의 30·90일 전 대비 변화율(일별 스냅샷 차분). <b>기준 시리즈 = 가장 가까운 미래 추정 분기</b>(예: 지금은 2026/09(E)) 영업익 컨센 하나 — 연간(FY)·타 분기가 아니라 같은 분기끼리 비교한다. <b>30일: 스냅샷이 30일 미만이면 쌓인 만큼(최소 7일, 적재 시작 2026-08-09) 부분 창으로 제공 — 09-08부터 정식 30일 창</b>. <b>90일=2026-11-07~ 유효</b>(부분 창 없음)'],
        ...
        ['전망분기·전망연간 전구간↑','한국 전용. WISEreport 컨센 표에서 <b>모든 미래(E) 구간의 영업이익 증가율이 양수</b>인가 — 분기(E)는 전년동기(YoY) 대비, 연간(E)은 전년 대비(예: 2026/09E·2026/12E·2027/03E 전부 + / FY2026E·2027E·2028E 전부 +). 기저가 결측·0 근처인 구간은 판정에서 제외 · 적자 기저의 흑자전환은 +로 본다. 두 필터를 함께 켜면 \'분기로도 연간으로도 꺾이는 구간이 없는\' 전망 종목만 남는다'],
  ...
  const GCAT=[['시세',['px','chg','cap','tv','turn']],['기간수익률',['r1m','r3m','r6m','mom']],
    ...
    ['컨센서스',['ern','tp','upside','recn','rev','nan']],
      ['실적발표',['edld','spr','sspr','sprb','cr7','cr30','cr90','qup','yup','pr7','pr30','pr90','tprv','tprv90','gap','gapP','gapE','gapEP','r1','r20']],
...
await _canvasFlow(c);
  ...
  /* (2026-09-02) 목표주가 변동표 + 30/90일 평균 + 발간일 산점도 — KR(WISEreport cTB24)·US(Yahoo
     upgradeDowngradeHistory) 공용. tp 행 스키마 {b,d,tp,prev,chg,op[,act]} · cur=현재가 · el=폭 측정용.
     o.fm 숫자 포맷(US 는 $·소수1자리) · o.title/o.unit/o.note 문구. */
  function _tpRevBlock(tpRaw, cur, el, o){
    o=o||{};
    const FM=o.fm||(v=>Math.round(v).toLocaleString());
    const P=v=>v==null?'<span class="note">—</span>':`<span class="${v>0?'up':(v<0?'dn':'note')}">${v>0?'+':''}${v.toFixed(1)}%</span>`;
    let t2='';
    const tp=(tpRaw||[]).slice().sort((a,b)=>a.d<b.d?-1:1);
    if(tp.length){
      ...
      let avgLine='';
      {
        ...
        const fmt=o2=>{ if(!o2) return null; const o=o2;
          const gp=(cur&&cur>0)?((o.v/cur-1)*100):null;
          return `<b>${FM(o.v)}</b><span class="note">(${o.n}건 · <span class="up">상향 ${o.u}</span>·<span class="dn">하향 ${o.d}</span>)</span>`+
      ...
      t2=`<div style="margin-top:10px"><b style="font-size:12px">${o.title||'목표주가 변동'} (${o.unit||'증권사 리포트'} 최근 ${tp.length}건 · ${span})</b>
        <span class="note">${o.note||'소스가 최근 ~24건만 보관 — 리포트 많은 종목은 90일 전체가 아닐 수 있음'}</span> ${avgLine}
        ...
        tp.slice().reverse().map(z=>`<tr style="border-bottom:1px solid #f4f6f8"><td style="padding:2px 4px">${z.d}</td><td>${E(z.b)}</td>
          <td style="text-align:right">${z.tp==null?'—':FM(z.tp)}</td><td style="text-align:right;color:var(--tx2)">${z.prev==null?'—':FM(z.prev)}</td>
          <td style="text-align:right">${P(z.chg)}</td><td style="text-align:center;font-size:10px">${E(z.op||'')}${z.act&&z.act!=='main'&&z.act!=='init'?` <span class="${z.act==='up'?'up':(z.act==='down'?'dn':'note')}">${z.act==='up'?'▲':(z.act==='down'?'▼':'')}</span>`:''}</td></tr>`).join('')+'</table></div>'+svg+'</div>';
    }
    return t2;
  }
  async function loadKrFin(c){
    ...
    /* ② 목표주가 변동표 + 미니 그래프 — 공용 _tpRevBlock (2026-09-02 분리) */
    const t2=_tpRevBlock(J.tp, (_CD&&_CD.c)?[..._CD.c].reverse().find(v=>v!=null):null, el);
    ...
    if(sn.length){
      ...
      t3=`<div style="margin-top:10px"><b style="font-size:12px">영업이익 컨센서스 리비전 (90일)</b> <span class="note">(일별 스냅샷 · 억원 · %는 직전 스냅샷 대비 · 스크리너의 리비전 30·90일 판정은 이 표의 <b>가장 가까운 미래 추정 분기</b> 열 하나로 계산 — 같은 분기끼리 N일 전 대비)</span>
  ...
  async function loadUsFin(c){
    ...
    { const rw=(POOL.us||[]).find(z=>z.c===c)||{};
      ...
      /* (2026-09-02) 증권사별 목표가 변동 — KR 과 같은 표·그래프. 소스 Yahoo upgradeDowngradeHistory
         (최근 120일 · 목표가 없는 등급변경 행은 '—'). 실측 A 8/27: 실적 후 8곳 일제 상향 155→180 등. */
      t2+=_tpRevBlock(J.tp, cur, el, {fm:v=>'$'+(+v).toFixed(v>=1000?0:1), unit:'증권사 리포트', title:'목표주가 변동',
        note:'Yahoo 등급·목표가 변경 이력(최근 120일) — 목표가 미공개 등급변경은 "—" · ▲▼ = 투자의견 상향/하향'});
    }
  ...
  const _KRREV=()=>{
    /* (2026-09-02) 날짜 게이트 제거 — 30일 리비전은 부분 창(쌓인 만큼)으로 즉시 제공되므로
       데이터 존재 여부로만 판단한다. 90일은 부분 창 없이 2026-11-07~ 자동. */
    const has=(POOL&&POOL.kr||[]).some(r=>r.cr30!=null);
    return has?'cr30':'tprv';
  };
