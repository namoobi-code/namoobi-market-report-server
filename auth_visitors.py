...
def _ua_short(ua: str) -> str:
    if not ua or ua == "-":
        return "?"
    os_ = ("Windows" if "Windows" in ua else "Android" if "Android" in ua else
           "iPhone" if "iPhone" in ua else "iPad" if "iPad" in ua else
           "Mac" if "Macintosh" in ua else "Linux" if "Linux" in ua else "")
    br = ""
    for pat, name in (("Edg/", "Edge"), ("OPR/", "Opera"), ("Whale", "Whale"),
                      ("Firefox/", "Firefox"), ("Chrome/", "Chrome"), ("Safari/", "Safari")):
        if pat in ua:
            br = name
            break
    m = re.search(r"(?:Chrome|Firefox|Version)/(\d+)", ua)
    if br and m:
        br += " " + m.group(1)
    return (os_ + " · " + br).strip(" ·") or ua[:40]
